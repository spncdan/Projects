...
int main() {
    ...
    string hard_words[10] = { "backpack", "daughter", "hospital", "keyboard", "magazine", "sandwich", "treasure", "umbrella", "victory", "notebook" };
    string insane_words[10] = { "collaboration", "companionship", "distribution", "environment", "investigate", "manufacturing", "participation", "preservation", "transportation", "understanding" };
    ...
    while (menu_exit == false) {
        ...
        while (!(cin >> menu_option)) {
            ...
            cout << "Invalid input. Please enter a number: "; // Asking user to input a number
        }
        ...
        switch (menu_option) {
            case 1: { // Commences the game
                ...
                while (keepgoing == true) {
                    ...
                    do { // do while loop which will loop if the letterguessed function is true.
                        cout << "Please enter a letter: " << endl;
                        cin >> guessed_letter; // Allowing player to input their letter guess
                        if (cin.fail()) { // Error checking for invalid inputs (sourced from chatgpt when error checking and seeing user could cause fail state)
                            cin.clear(); // Clear the error state
                            cin.ignore(numeric_limits<streamsize>::max(), '\n'); // Ignore invalid input
