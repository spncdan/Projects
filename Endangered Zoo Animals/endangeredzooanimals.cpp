// This a program aims to bring in contents from a data file and classify them in relation to zoo animals which may or may not need attention by the zoo.
...
bool attention(int x, char y){
    if(x < 10 and y == 'y'){ // if statement which will return true if x is less than 10 AND they are checked off as "y" in file
        return true;
    }
    else
